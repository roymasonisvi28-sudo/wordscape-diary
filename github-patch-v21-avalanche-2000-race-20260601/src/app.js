(function () {
  ...
  const defaultState = () => ({
    ...
    avalanche: { encounters: 0, wins: 0, losses: 0, checkedChapters: {} },
  ...
  function avalancheArchiveCard() { const a = state.avalanche; return `<article class="banana-profile avalanche-profile"><div class="banana-profile-image"><img src="${avalancheImage()}" alt="张雪崩老师"></div><div><small>从天而降的赛跑挑战者</small><h3>张雪崩老师 <em>小游戏彩蛋</em></h3><p>他会突然打断主线，把画面变成十秒横版赛道。连续答对五道核心词题，才能在终点前跑赢他。</p><p class="meta">已经乱入 ${a.encounters} 次 · 获胜 ${a.wins} 次 · 失利 ${a.losses} 次<br>主题：核心词汇、十秒竞速、Q 萌平台赛道</p></div></article>`; }
  ...
  function nextLine() { clearTimers(); if (++line < currentChapter.dialogue.length) { state.checkpoint = { chapter: currentChapter.id, stage: "story", line, choiceRound: 0 }; store(); return story(); } if (maybeAvalancheEncounter() || maybeBananaEncounter()) return; choice(); }
  ...
  function maybeAvalancheEncounter() {
    ...
    if (Math.random() > .24) return false;
  ...
  function finishAvalancheRace() {
    ...
    state.avalanche[won ? "wins" : "losses"]++; store();
